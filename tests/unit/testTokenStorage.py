...
class Abstract:
    ...
    class TokenStorageTest(TwistedTestCase):
        ...
        call setupTokenStorage with an instance of the token storage to test.
        """
        _TOKEN_STORAGE = None
        ...
        def testHasAccess(self):
            ...
            self.assertTrue(
                self._TOKEN_STORAGE.hasAccess(self._VALID_TOKEN, self._VALID_SCOPE[0:1]),
            ...
            self.assertFalse(
                self._TOKEN_STORAGE.hasAccess(self._VALID_TOKEN,
                                              self._VALID_SCOPE + ['invalidScope']),
        ...
        def testTokenOverwrite(self):
            ...
            self.assertEqual(
                self._DUMMY_CLIENT.id, self._TOKEN_STORAGE.getTokenClient(token),
                msg='Expected getTokenScope to return the new client id '
                    'that was stored with the token.')
...
class DictTokenStorageTest(Abstract.TokenStorageTest):
