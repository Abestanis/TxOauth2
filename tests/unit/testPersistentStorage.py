...
class AbstractPersistentStorageTest(TwistedTestCase):
    """
    An abstract test case for PersistentStorage implementations. A subclass must set __test__
    to True and call setupPersistentStorage with an instance of the persistent storage to test.
    """
    ...
    def testPut(self):
        ...
        for dataKey, dataValue in data.items():
            self._assertNotContains(dataKey, msg='Expected that the persistent storage would not '
...
class DictNonPersistentStorageTest(AbstractPersistentStorageTest):
